Ext.define('moneyworld.controller.DetailedViewGini', {
	extend: 'moneyworld.controller.DetailedView',
	
	config: {
		refs: {
			mainView: 'main',
			detailedViewGini: 'detailedview_gini',
		},
		control: {
			'detailedViewGini': {
	...
	renderView: function() {
		...
		function loadDataPointsStore(records, operation, success) {
			...
			dataPointsStore = moneyworld.utils.Functions.getServerStore(
				this.getDetailedViewGini().getDataSet(),
				"all",
				currentCountry);
		...
		function setData(records, operation, success) {
					...
					axes: [
						{
							type: 'numeric',
							position: 'left',
							title: dataSetsStore.findRecord('id', this.getDetailedViewGini().getDataSet()).get('name'),
							grid: true,
							style: {
								estStepSize: 20
							},
			...
			this.getDetailedViewGini().add(chart);
