Ext.define('moneyworld.controller.DetailedViewGdp', {
	extend: 'moneyworld.controller.DetailedView',
	
	config: {
		refs: {
			mainView: 'main',
			detailedViewGdp: 'detailedview_gdp',
		},
		control: {
			'detailedViewGdp': {
	...
	renderView: function() {
		...
		function loadDataPointsStore(records, operation, success) {
			...
			dataPointsStore = moneyworld.utils.Functions.getServerStore(
				this.getDetailedViewGdp().getDataSet(),
				"all",
				currentCountry);
		...
		function setData(records, operation, success) {
					...
					axes: [
						{
							type: 'numeric',
							position: 'left',
							title: dataSetsStore.findRecord('id', this.getDetailedViewGdp().getDataSet()).get('name'),
							grid: true,
							style: {
								estStepSize: 20
							},
			...
			this.getDetailedViewGdp().add(chart);
