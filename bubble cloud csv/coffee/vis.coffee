...
Bubbles = () ->
  ...
  updateActive = (id) ->
    ...
    if id.length > 0
      d3.select("#status").html("<h3>The country <span class=\"active\">#{id}</span> is now active</h3>")
    else
      d3.select("#status").html("<h3>No country is active</h3>")
...
texts = [
  {key:"2012",file:"2012_something.csv",name:"2012 something"}
